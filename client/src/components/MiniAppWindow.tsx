...
const MiniAppWindow: React.FC<MiniAppWindowProps> = ({ app, onClose, onMinimize, minimized = false }) => {
    ...
    useEffect(() => {
        ...
        const respond = (id: string, payload: any, transfer: Transferable[] = []) => {
            try { iframe.contentWindow?.postMessage({ __zvon: true, id, ...payload }, '*', transfer); } catch {}
        };
        ...
        const handleMsg = async (msg: any, sourcePort?: MessagePort) => {
            ...
            try {
                switch (type) {
                    ...
                    case 'store': {
                        // VPN-магазин: проксируем на собственный store-бэкенд Zvon с
                        // авторизацией текущего пользователя. Разрешено только системному
                        // мини-аппу магазина (по URL) — чтобы сторонние аппы не читали
                        // ключи и не оформляли заказы от имени пользователя.
                        if (!absoluteUrl.includes('/miniapps/vpn-store')) {
