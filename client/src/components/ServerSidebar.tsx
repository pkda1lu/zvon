...
const ServerSidebar: React.FC<ServerSidebarProps> = ({
  ...
  const handleUserDragStart = (e: React.DragEvent, userId: string, fromChannelId: string) => {
    ...
    try { e.dataTransfer.setData('application/zvon-voice-user', payload); } catch {}
  ...
  const handleChannelDrop = (e: React.DragEvent, channelId: string) => {
    ...
    const raw =
      e.dataTransfer.getData('application/zvon-voice-user')
  ...
  // 3D-комнаты используют тот же голосовой/LiveKit-стек, что и голосовые каналы,
  // поэтому рендерятся тем же renderVoiceLikeChannels, но в своей категории.
  ...
  // Категория голосовых-подобных каналов (голосовые и 3D-комнаты). Оба типа
  // используют один voice/LiveKit-стек, поэтому рендер общий — различаются только
  // заголовок категории и иконка канала.
