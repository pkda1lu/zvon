...
const VlyneIdDevCabinet = React.lazy(() => import('./pages/VlyneIdDevCabinet'));
 ...
 *
 * Отличается от той, что VibeBackground берёт по умолчанию для страниц
 * Vlyne ID, и подбиралась под конкретное условие: здесь почти весь экран
 * закрыт стеклянными панелями, и фон виден в основном СКВОЗЬ них — через
 * backdrop-filter: blur(28px) и подложку --panel-overlay-*. Стекло съедает
 * рисунок, поэтому палитра, отлично выглядящая на открытом фоне, под панелью
 * превращается в ровное пятно.
 *
 * Поэтому проверять её нужно на макете с почти полноэкранной панелью, а не на
 * трёх карточках с широкими промежутками: на втором рисунок виден всегда, и
 * легко выбрать слишком тёмный вариант.
 *
 * Здесь основа почти чёрная, а цвет держится в двух верхних слоях: хватает,
 * чтобы завихрения читались и по краям, и приглушённо под панелями, но текст
 * на панели не теряет контраст. Хочется сильнее — в настройках есть
 * «Непрозрачность панелей», она делает подложку прозрачнее.
 */
    ...
    <div style={{ position: 'relative', flex: 1, minHeight: 0, width: '100%' }}>
      <AnimatePresence mode="wait" initial={false}>
        ...
        <Routes location={location} key={routeKey}>
          ...
          <Route path="/vlyneid/developers/cabinet" element={<PageShell><VlyneIdDevCabinet /></PageShell>} />
          ...
          {isVlyneIdHost && (
            <Route path="/developers/cabinet" element={<PageShell><VlyneIdDevCabinet /></PageShell>} />
          )}
