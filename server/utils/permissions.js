...
const canPerformActionOn = async (actorId, targetId, serverId) => {
    try {
        ...
        const actor = server.members.find(m => m.user.toString() === actorId.toString());
        const target = server.members.find(m => m.user.toString() === targetId.toString());

        if (!actor || !target) return false;

        // Get highest role position for actor
        const actorRoleIds = actor.roles.map(r => r.toString());
        const actorRoles = server.roles.filter(r => actorRoleIds.includes(r._id.toString()));
        const actorHighest = actorRoles.length > 0 ? Math.max(...actorRoles.map(r => r.position)) : -1;

        // Get highest role position for target
        const targetRoleIds = target.roles.map(r => r.toString());
        const targetRoles = server.roles.filter(r => targetRoleIds.includes(r._id.toString()));
        const targetHighest = targetRoles.length > 0 ? Math.max(...targetRoles.map(r => r.position)) : -1;
