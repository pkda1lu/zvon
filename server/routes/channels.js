...
router.post('/', auth, checkPermission(Permissions.MANAGE_CHANNELS, 'body.serverId'), async (req, res) => {
  try {
    ...
    if (io) {
      const updatedServer = await Server.findById(serverId).populate('owner', 'username avatar badges').populate('channels').populate('members.user', 'username avatar status badges');
...
router.put('/:id', auth, async (req, res, next) => {
  ...
  try {
    ...
    const io = req.app.get('io');
    if (io) {
      const updatedServer = await Server.findById(channel.server).populate('owner', 'username avatar badges').populate('channels').populate('members.user', 'username avatar status badges');
