...
# Скрипт автоматического развертывания Zvon Server на Ubuntu VPS
# Использование: sudo bash deploy.sh

set -e

echo "=========================================="
echo "Zvon Server - Автоматическое развертывание"
echo "=========================================="
...
cat > /etc/nginx/sites-available/zvon << 'NGINX_CONFIG'
server {
    listen 80;
    server_name _;
...
if [ ! -L /etc/nginx/sites-enabled/zvon ]; then
    ln -s /etc/nginx/sites-available/zvon /etc/nginx/sites-enabled/
...
echo "   cd $SERVER_DIR"
echo "   pm2 start server.js --name zvon-server"
echo "   pm2 save"
echo "   pm2 startup"
echo ""
